...
- **2026-04-21** — Package renamed `fairscape_interpret` → `fairscape_graph_tools` (dir, `pyproject.toml` name, Python module, all import sites in `mds_python` and `fairscape-cli`) to match the GitHub repo rename and cover the broader scope now landing (evidence-graph builder, not only AI interpretation). Full rename in one coordinated commit per repo rather than a shim-based compat layer — rename surface is small and well-known from the just-finished interpret migration, so lingering dual-name state isn't worth the maintenance cost. Historical decision entries in this doc still reference `fairscape_interpret`; don't back-edit them. New tracking doc for the follow-up evidence-graph work is `./EVIDENCE_GRAPH_MIGRATION.md`.
